...
export class VariantResolver {
  ...
  @Query(() => Variant, { name: 'variant', description: 'Retrieves a specific variant by ID' })
  async getVariant(@Args('variantId', { type: () => String }) variantId: string) {
    return this.variantService.getVariant(variantId);
  }

  @Query(() => [Variant], { name: 'productVariants', description: 'Retrieves all variants for a product' })
  ...
  @Roles(Role.ADMIN)
  @Query(() => [Variant], { name: 'variants', description: 'Retrieves all variants (admin only)' })
  async getAllVariants() {
    return this.variantService.getAllVariants();
  }

  @Roles(Role.ADMIN)
  @Mutation(() => Variant, { name: 'createVariant', description: 'Creates a new variant for a product' })
  async createVariant(@Args('input') input: CreateVariantInput) {
    return this.variantService.createVariant(input);
  }

  @Roles(Role.ADMIN)
  @Mutation(() => Variant, { name: 'updateVariant', description: 'Updates an existing variant' })
  async updateVariant(@Args('input') input: UpdateVariantInput) {
    return this.variantService.updateVariant(input.id, input);
  }

  @Roles(Role.ADMIN)
  @Mutation(() => Variant, { name: 'deleteVariant', description: 'Deletes a variant' })
